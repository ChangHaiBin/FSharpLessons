...
let GetMeal money =
    if money > 59.90 then "LARGE SET"
    else if money >= 39.90 then "MEDIUM SET"
    else "SMALL SET"

let GetNumberPeople meal =
    if meal = "LARGE SET" then 6
    else if meal = "MEDIUM SET" then 4
    else 2


// GetMeal:         float -> string
// GetNumberPeople:          string -> int

// Long Version:
let NumFriendsToInvite money =
    let intermediateResult = GetMeal money
    let finalResult = GetNumberPeople intermediateResult
    // return
    finalResult

// Try to write the short version using |>

let NumFriendsToInvite2 money =
    money
    |> ......
    // ......
...
let func1 x = (x < 0)
let func2 y = if y = true then 1.0 else 5.0
let func3 z = if z < 3.0 then "SMALL" else "LARGE"

// func1 : int -> bool
// func2 :        bool -> float
// func3 :                float -> string

let CombinedFunction1 x =
    let intermediateResult1 = func1 x
    let intermediateResult2 = func2 intermediateResult1
    let finalResult = func3 intermediateResult2
    // return 
    finalResult

let CombinedFunction2 x =
    x
    |> ......
    ......
...
let numShares1 = GetNumSharesFromEstimate2 [4.0; 5.0; 3.0; 2.0; 2.5]
...
let numShares2 = GetNumSharesFromEstimate2 [8.0; 7.0; 10.0; 12.0; 10.5]
